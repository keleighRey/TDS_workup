---
title: "LMAS/SMAS TDS & Chloride Analysis (Median Values)"
...
Stream and lake data collected since 2001 were summarized by grouping by each site (stream site or lake) and the **median** value over the period of the record was calculated. This was done to reduce the influence of waterbodies which were sampled multiple times skewing the resulting summary statistics. For lake data, epilimnetic and hypolimnetic data were analyzed separately. 
...
tds_ecoregions_stats<-merged_tds %>%
  ...
  mutate(median_TDS = median(TOTAL.DISSOLVED.SOLIDS, na.rm = TRUE)) %>% 
  ungroup() %>% 
  distinct(event_smas_history_id, median_TDS, .keep_all = TRUE) %>% 
  group_by(US_L3CODE) %>% 
  mutate(median = median(median_TDS, na.rm = TRUE)) %>%
  mutate(q_25 = quantile(median_TDS, probs = 0.25, na.rm = TRUE)) %>%
  mutate(q_80 = quantile(median_TDS, probs = 0.80, na.rm = TRUE)) %>% 
  mutate(q_75 = quantile(median_TDS, probs = 0.75, na.rm = TRUE)) %>% 
  mutate(q_95 = quantile(median_TDS, probs = 0.95, na.rm = TRUE)) %>%
  mutate(max = max(median_TDS, na.rm = TRUE)) %>% 
  ungroup() %>%
  group_by(US_L3CODE) %>% 
  mutate(n = n()) %>% 
  ungroup() %>% 
...
predicted_stream_TDS_ecoregion_stats<-merged_conduct_all_streams %>%
  ...
  group_by(event_smas_history_id) %>%
  mutate(median_TDS = median(pred_TDS, na.rm = TRUE)) %>% 
  ungroup() %>% 
  distinct(event_smas_history_id, median_TDS, .keep_all = TRUE) %>% 
  group_by(US_L3CODE) %>% 
  mutate(median = median(median_TDS, na.rm = TRUE)) %>% 
...
lakes_cond_epi<-lakes_cond %>% 
  ...
  mutate(median_spc = median(RSLT_RESULT_VALUE, na.rm = TRUE)) %>% 
  ungroup() %>% 
...
predicted_lake_epi_TDS_ecoregion_stats<-lakes_TDS_epi %>%
  ...
  mutate(q_25 = quantile(pred_median_TDS, probs = 0.25, na.rm = TRUE)) %>% 
  mutate(q_80 = quantile(pred_median_TDS, probs = 0.80, na.rm = TRUE)) %>% 
  mutate(q_75 = quantile(pred_median_TDS, probs = 0.75, na.rm = TRUE)) %>% 
  mutate(q_95 = quantile(pred_median_TDS, probs = 0.95, na.rm = TRUE)) %>% 
  mutate(max = max(pred_median_TDS, na.rm = TRUE)) %>% 
  ungroup()%>%
  group_by(US_L3CODE) %>% 
  mutate(n = n()) %>% 
  ungroup() %>% 
...
predicted_lake_epi_TDS_classification_stats<-lakes_TDS_epi %>%
  ...
  group_by(LOCATION_WATERBODY_CLASSIFICATION) %>% 
  mutate(median = median(pred_median_TDS, na.rm = TRUE)) %>% 
  mutate(q_25 = quantile(pred_median_TDS, probs = 0.25, na.rm = TRUE)) %>% 
  mutate(q_80 = quantile(pred_median_TDS, probs = 0.80, na.rm = TRUE)) %>% 
  mutate(q_75 = quantile(pred_median_TDS, probs = 0.75, na.rm = TRUE)) %>% 
  mutate(q_95 = quantile(pred_median_TDS, probs = 0.95, na.rm = TRUE)) %>%
...
stream_chloride_ecoregions_stats<-merged_chloride %>%
  ...
  mutate(q_95 = quantile(median_chloride, probs = 0.95, na.rm = TRUE)) %>% 
  mutate(max = max(median_chloride, na.rm = TRUE)) %>% 
  ungroup() %>%
  group_by(US_L3CODE) %>% 
  mutate(n = n()) %>% 
  ungroup() %>% 
 ...
 stream_chloride_ecoregion_plot<-merged_chloride %>%
  filter(!is.na(US_L3NAME)) %>% 
  group_by(event_smas_history_id) %>%
  mutate(median_chloride = median(CHLORIDE, na.rm = TRUE)) %>% 
  ungroup() %>% 
  distinct(event_smas_history_id, median_chloride, .keep_all = TRUE) %>% 
...
```
## Relationship between TDS and Chloride in Streams

There is a strong relationship between chloride and TDS in streams, however, the variance increases as values increase. Some sites have elevated TDS, but not co-contaminant chloride.  
```{r, TDS vs. Chloride, message=FALSE, echo=FALSE, warning=FALSE, fig.cap=c("Figure 17: Linear regression of TDS and Chloride", "Figure 18: Linear regression of TDS and Chloride with reduced scale range")}

merged_chloride_tds<- read_csv(file.path(here::here(),
                              "data",
                              "merged_chloride_tds.csv"))
merged_chloride_tds<-merged_chloride_tds %>% 
  filter(TOTAL.DISSOLVED.SOLIDS<6000)

model2<-lm(merged_chloride_tds$CHLORIDE~merged_chloride_tds$TOTAL.DISSOLVED.SOLIDS)


merged_chloride_tds_plot1<-merged_chloride_tds %>% 
  ggplot(aes(TOTAL.DISSOLVED.SOLIDS, CHLORIDE))+ 
  geom_point()+
  xlab("TDS (mg/L)")+ylab("Chloride (mg/L)")+
  coord_cartesian(xlim = c(0,6000), ylim = c(0, 6000))+
  annotate("text", x = 2000, y = 4000, label = "R-squared = 0.6661, p-value <2e-16 ")

merged_chloride_tds_plot1

merged_chloride_tds_plot2<-merged_chloride_tds %>% 
  ggplot(aes(TOTAL.DISSOLVED.SOLIDS, CHLORIDE))+ 
  geom_point()+
  xlab("TDS (mg/L)")+ylab("Chloride (mg/L)")+
  coord_cartesian(xlim = c(0,2000), ylim = c(0, 1000))

merged_chloride_tds_plot2
```


```{r, NLCD Statistics Level III Ecoregions, results='hide', message=FALSE, echo=FALSE, warning=FALSE}

lulc<- read_csv(file.path(here::here(),
                              "data",
                              "NLCD_2016_ecoregions.csv"))

impervious<- read_csv(file.path(here::here(),
                              "data",
                              "NLCD_2016_Impervous.csv"))
```
## Level III Ecoregion Land Cover / Land Use Characteristics
...
kable(
  lulc,
  digits = 4,
  caption = "Table 14: Level III Ecoregion Land Cover (%)"
  ) %>% 
   kable_styling(position = "center")
...
kable(
  impervious,
  digits = 4,
  caption = "Table 15: Level III Ecoregion Percent Impervious Cover Statistics"
  ) %>% 
   kable_styling(position = "center")
...
## Median Stream TDS Values by Site 

```{r, SMAS median actual and predicted TDS vales, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
merged_tds <- read_csv(file.path(here::here(),
                              "data",
                              "merged_tds.csv"))
median_actual_tds<-merged_tds %>% 
  group_by(event_smas_history_id) %>%
  mutate(median = median(TOTAL.DISSOLVED.SOLIDS, na.rm = TRUE)) %>% 
  distinct(event_smas_history_id, median, .keep_all = TRUE) %>% 
  select(event_smas_history_id, L3_KEY, median, latitude, longitude) %>% 
  rename("Site ID" = event_smas_history_id, "Median TDS (mg/L)" = median, "Ecoregion Number / Name" = L3_KEY)
...
median_predicted_tds<-merged_conduct_all_streams %>% 
  group_by(event_smas_history_id) %>%
  mutate(median = median(pred_TDS, na.rm = TRUE)) %>% 
  distinct(event_smas_history_id, median, .keep_all = TRUE) %>% 
  select(event_smas_history_id, L3_KEY, median, latitude, longitude) %>% 
  rename("Site ID" = event_smas_history_id, "Predicted Median TDS (mg/L)" = median, "Ecoregion Number / Name" = L3_KEY)
...
datatable(median_actual_tds, filter = 'top', caption = "Table 16: Median Actual TDS by Site",  extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

datatable(median_predicted_tds, filter = 'top', caption = "Table 17: Median predicted TDS by Site", extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))
...
```{r, print max and actual SMAS TDS maps, echo=FALSE, warning=FALSE, message=FALSE, fig.cap= c("Figure 19: Map of actual median TDS by site", "Figure 20: Map of predicted median TDS by site")}

shapeData$L3_KEY <- factor(shapeData$L3_KEY)

factpal <- colorFactor(domain = shapeData$L3_KEY, palette = "Paired")


my_bins <-c(0, 10, 100, 250, 500, 1000, 2000, 3000, 4000, Inf) 
my_pal<-colorBin(palette = "Spectral", bins = my_bins, domain = median_actual_tds$`Median TDS (mg/L)`, reverse = TRUE)
  ...
  addCircleMarkers( radius = 2, opacity = 1,
label = ~htmlEscape(`Site ID`),
popup = ~paste0("<br/>Site ID:",`Site ID`,"<br/>TDS (mg/L):", `Median TDS (mg/L)`),
color = ~my_pal(median_actual_tds$`Median TDS (mg/L)`)) %>% 
    addLegend("bottomright", pal = my_pal, values = ~`Median TDS (mg/L)`,
    title = "Median TDS (mg/L)",
    labFormat = labelFormat(),
    opacity = 1
  )

my_bins <-c(0, 10, 100, 250, 500, 1000, 2000, 3000, 4000, Inf) 
my_pal<-colorBin(palette = "Spectral", bins = my_bins, domain = median_predicted_tds$`Predicted Median TDS (mg/L)`, reverse = TRUE)
  
  median_predicted_tds %>% 
  leaflet() %>%
  addPolygons(data=shapeData,weight=2,color = factpal(shapeData$L3_KEY), fillOpacity = 0.1, popup = paste("Ecoregion # / Name: ", shapeData$L3_KEY, "<br>")) %>% 
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers( radius = 2, opacity = 1,
label = ~htmlEscape(`Site ID`),
popup = ~paste0("<br/>Site ID:",`Site ID`,"<br/>Predicted Median TDS (mg/L):", `Predicted Median TDS (mg/L)`),
color = ~my_pal(median_predicted_tds$`Predicted Median TDS (mg/L)`)) %>% 
    addLegend("bottomright", pal = my_pal, values = ~`Predicted Median TDS (mg/L)`,
    title = "Predicted Median TDS (mg/L)",
    labFormat = labelFormat(),
    opacity = 1
  )

```
## Median Predicted Lake TDS Values by Site 
...
lakes_cond_epi<-lakes_cond %>% 
  ...
  mutate(median_spc = median(RSLT_RESULT_VALUE, na.rm = TRUE)) %>% 
  ungroup() %>% 
  distinct(LAKE_HISTORY_ID, median_spc, .keep_all = TRUE)

lakes_TDS_epi<-lakes_cond_epi %>% 
  mutate(pred_median_TDS=3.965894+(0.563565*median_spc))


lakes_TDS_epi_median<-inner_join(lakes_TDS_epi, lake_ecoregions, by = "LAKE_HISTORY_ID")

lakes_TDS_epi_median<-lakes_TDS_epi_median %>% 
  select(LAKE_HISTORY_ID, pred_median_TDS, LOCATION_Y, LOCATION_X, L3_KEY) %>% 
  rename("Site ID" = LAKE_HISTORY_ID, "Median Predicted TDS (mg/L)" = pred_median_TDS, "Ecoregion Number / Name" = L3_KEY, latitude = LOCATION_Y, longitude = LOCATION_X)


lakes_cond_hypo_median<-lakes_cond %>%
  select(LAKE_HISTORY_ID, SAMPLE_DATE, RSLT_RESULT_VALUE, RSLT_PROFILE_DEPTH) %>%
  filter(RSLT_PROFILE_DEPTH > 5.0) %>% 
  group_by(LAKE_HISTORY_ID, SAMPLE_DATE) %>% 
  mutate(max_profile_depth = max(RSLT_PROFILE_DEPTH)) %>% 
...
lakes_cond_hypo_median<-lakes_cond_hypo_median %>% 
  select(LAKE_HISTORY_ID, RSLT_RESULT_VALUE, RSLT_PROFILE_DEPTH, SAMPLE_DATE) %>% 
  group_by(LAKE_HISTORY_ID) %>% 
  mutate(median_spc = median(RSLT_RESULT_VALUE, na.rm = TRUE)) %>% 
  ungroup() %>% 
...
lakes_TDS_hypo_median<-lakes_cond_hypo_median %>% 
  mutate(pred_median_TDS=3.965894+(0.563565*median_spc))

lakes_TDS_hypo_median<-inner_join(lakes_TDS_hypo_median, lake_ecoregions, by ="LAKE_HISTORY_ID")

lakes_TDS_hypo_median<-lakes_TDS_hypo_median %>% 
  select(LAKE_HISTORY_ID, pred_median_TDS, LOCATION_Y, LOCATION_X, L3_KEY) %>% 
  rename("Site ID" = LAKE_HISTORY_ID, "Median Predicted TDS (mg/L)" = pred_median_TDS, "Ecoregion Number / Name" = L3_KEY, latitude = LOCATION_Y, longitude = LOCATION_X)
...
datatable(lakes_TDS_epi_median, filter = 'top', caption = "Table 21: Median predicted epilimnetic TDS by site",  extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

datatable(lakes_TDS_hypo_median, filter = 'top', caption = "Table 22:Median predicted hypolimnetic TDS by site", extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

```

```{r, print max LMAS max predicted TDS maps epi and hypo, echo=FALSE, warning=FALSE, message=FALSE, fig.cap= c("Figure 21: Map of predicted median epilimnetic TDS by site", "Figure 22:  Map of predicted median hypolimnetic TDS by site")}


my_bins <-c(0, 10, 100, 250, 500, 1000, 2000, 3000, 4000, Inf) 
my_pal<-colorBin(palette = "Spectral", bins = my_bins, domain = lakes_TDS_epi_median$`Median Predicted TDS (mg/L)`, reverse = TRUE)
  
  lakes_TDS_epi_median %>% 
  leaflet() %>%
  addPolygons(data=shapeData,weight=2,color = factpal(shapeData$L3_KEY), fillOpacity = 0.1, popup = paste("Ecoregion # / Name: ", shapeData$L3_KEY, "<br>")) %>% 
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers( radius = 2, opacity = 1,
label = ~htmlEscape(`Site ID`),
popup = ~paste0("<br/>Site ID:",`Site ID`,"<br/>Predicted Median TDS (mg/L):", `Median Predicted TDS (mg/L)`),
color = ~my_pal(lakes_TDS_epi_median$`Median Predicted TDS (mg/L)`)) %>% 
    addLegend("bottomright", pal = my_pal, values = ~`Median Predicted TDS (mg/L)`,
    title = "Predicted Median TDS (mg/L)",
    labFormat = labelFormat(),
    opacity = 1
  )
  ...
  my_bins <-c(0, 10, 100, 250, 500, 1000, 2000, 3000, 4000, Inf) 
my_pal<-colorBin(palette = "Spectral", bins = my_bins, domain = lakes_TDS_hypo_median$`Median Predicted TDS (mg/L)`, reverse = TRUE)
  
    lakes_TDS_hypo_median %>% 
  leaflet() %>%
  addPolygons(data=shapeData,weight=2,color = factpal(shapeData$L3_KEY), fillOpacity = 0.1, popup = paste("Ecoregion # / Name: ", shapeData$L3_KEY, "<br>")) %>% 
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers( radius = 2, opacity = 1,
label = ~htmlEscape(`Site ID`),
popup = ~paste0("<br/>Site ID:",`Site ID`,"<br/>Predicted Median TDS (mg/L):", `Median Predicted TDS (mg/L)`),
color = ~my_pal(lakes_TDS_hypo_median$`Median Predicted TDS (mg/L)`)) %>% 
    addLegend("bottomright", pal = my_pal, values = ~`Median Predicted TDS (mg/L)`,
    title = "Predicted Median TDS (mg/L)",
    labFormat = labelFormat(),
    opacity = 1
...
## Median Stream Chloride Values by Site
```{r, SMAS median chloride vales, echo=FALSE, results='hide', message=FALSE, warning=FALSE }

merged_chloride<- read_csv(file.path(here::here(),
                              "data",
                              "merged_chloride.csv"))

stream_median_chloride<-merged_chloride %>% 
  group_by(event_smas_history_id) %>%
  mutate(median = median(CHLORIDE, na.rm = TRUE)) %>% 
  distinct(event_smas_history_id, median, .keep_all = TRUE) %>% 
  select(event_smas_history_id, L3_KEY, median, latitude, longitude) %>% 
  rename("Site ID" = event_smas_history_id, "Median Chloride (mg/L)" = median, "Ecoregion Number / Name" = L3_KEY)

```

```{r, print SMAS median chloride datatable, echo=FALSE, warning=FALSE, message=FALSE}

datatable(stream_median_chloride, filter = 'top', caption = "Table 23: Stream Median Chloride (mg/L) by Site",  extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))
```


```{r, print median SMAS chloride map, echo=FALSE, warning=FALSE, message=FALSE, fig.cap= "Figure 23: Map of median chloride by site"}

my_bins <-c(0, 10, 25, 50, 100, 200, 300, 400, 500, Inf) 
my_pal<-colorBin(palette = "Spectral", bins = my_bins, domain = stream_median_chloride$`Median Chloride (mg/L)`, reverse = TRUE)

  stream_median_chloride %>% 
  leaflet() %>%
  addPolygons(data=shapeData,weight=2,color = factpal(shapeData$L3_KEY), fillOpacity = 0.1, popup = paste("Ecoregion # / Name: ", shapeData$L3_KEY, "<br>")) %>% 
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers( radius = 2, opacity = 1,
label = ~htmlEscape(`Site ID`),
popup = ~paste0("<br/>Site ID:",`Site ID`,"<br/>Chloride (mg/L):", `Median Chloride (mg/L)`),
color = ~my_pal(stream_median_chloride$`Median Chloride (mg/L)`)) %>% 
    addLegend("bottomright", pal = my_pal, values = ~`Median Chloride (mg/L)`,
    title = "Median Chloride (mg/L)",
    labFormat = labelFormat(),
    opacity = 1
  )

```
## Median Lake Chloride Values by Site

```{r, median lake chloride by site, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
lake_ecoregions_chloride <- read_csv(file.path(here::here(),
                              "data",
                              "lake_ecoregions_chloride.csv"))
...
lakes_chloride_epi<-lakes_chloride_epi %>% 
  select(LAKE_HISTORY_ID, median_chloride, L3_KEY, LOCATION_Y_COORDINATE, LOCATION_X_COORDINATE) %>% 
  rename("Site ID" = LAKE_HISTORY_ID, "Median Chloride (mg/L)" = median_chloride, "Ecoregion Number / Name" = L3_KEY, latitude = LOCATION_Y_COORDINATE, longitude = LOCATION_X_COORDINATE)
...
```{r, print LMAS median chloride datatables epi and hypo, echo=FALSE, warning=FALSE, message=FALSE}

datatable(lakes_chloride_epi, filter = 'top', caption = "Table 24: Lake median epilimnetic chloride by site",  extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

datatable(lakes_chloride_hypo, filter = 'top', caption = "Table 25:Lake median hypolimnetic chloride by site", extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

```

```{r, print max LMAS median chloride maps epi and hypo, echo=FALSE, warning=FALSE, message=FALSE, fig.cap= c("Figure 25: Map median epilimnetic chloride by site", "Figure 26:  Map of median hypolimnetic chloride by site")}


my_bins <-c(0, 10, 25, 50, 100, 200, 300, 400, 500, Inf) 
my_pal<-colorBin(palette = "Spectral", bins = my_bins, domain = lakes_chloride_epi$`Median Chloride (mg/L)`, reverse = TRUE)
  ...
  addCircleMarkers( radius = 2, opacity = 1,
label = ~htmlEscape(`Site ID`),
popup = ~paste0("<br/>Site ID:",`Site ID`,"<br/>TDS (mg/L):", `Median Chloride (mg/L)`),
color = ~my_pal(lakes_chloride_epi$`Median Chloride (mg/L)`)) %>% 
    addLegend("bottomright", pal = my_pal, values = ~`Median Chloride (mg/L)`,
    title = "Epilimnetic Chloride (mg/L)",
    labFormat = labelFormat(),
    opacity = 1
  )
...
label = ~htmlEscape(`Site ID`),
popup = ~paste0("<br/>Site ID:",`Site ID`,"<br/>TDS (mg/L):", `Median Chloride (mg/L)`),
color = ~my_pal(lakes_chloride_hypo$`Median Chloride (mg/L)`)) %>% 
    addLegend("bottomright", pal = my_pal, values = ~`Median Chloride (mg/L)`,
    title = "Hypolimnetic Chloride (mg/L)",
    labFormat = labelFormat(),
    opacity = 1
  )
```    
