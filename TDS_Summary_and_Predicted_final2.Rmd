...
median_actual_tds<-merged_tds %>% 
  ...
  select(event_smas_history_id, event_smas_sample_date, L3_KEY, median, latitude, longitude) %>% 
  rename("Site ID" = event_smas_history_id, "Sample Date" = event_smas_sample_date, "Median TDS (mg/L)" = median, "Ecoregion Number / Name" = L3_KEY)
...
datatable(median_actual_tds, filter = 'top', caption = "Table 16: Maximum Actual TDS by Site",  extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

datatable(median_predicted_tds, filter = 'top', caption = "Table 17: Maximum predicted TDS by Site", extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

```


```{r, print max and actual SMAS TDS maps, echo=FALSE, warning=FALSE, message=FALSE, fig.cap= c("Figure 19: Map of actual maximum TDS by site", "Figure 20: Map of predicted maximum TDS by site")}
...
my_bins <-c(0, 10, 100, 250, 500, 1000, 2000, 3000, 4000, Inf) 
my_pal<-colorBin(palette = "Spectral", bins = my_bins, domain = median_predicted_tds$`Median TDS (mg/L)`, reverse = TRUE)
...
label = ~htmlEscape(`Site ID`),
popup = ~paste0("<br/>Site ID:",`Site ID`,"<br/>TDS (mg/L):", `Median TDS (mg/L)`),
color = ~my_pal(median_predicted_tds$`Median TDS (mg/L)`)) %>% 
    addLegend("bottomright", pal = my_pal, values = ~`Median TDS (mg/L)`,
...
lakes_TDS_epi_max<-lakes_TDS_epi_max %>% 
  ...
  rename("Site ID" = LAKE_HISTORY_ID, "Sample Date" = SAMPLE_DATE, "Max TDS (mg/L)" = pred_max_TDS, "Ecoregion Number / Name" = L3_KEY, latitude = LOCATION_Y, longitude = LOCATION_X)
...
datatable(lakes_TDS_epi_max, filter = 'top', caption = "Table 18: Maximum predicted epilimnetic TDS by site",  extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

datatable(lakes_TDS_hypo_max, filter = 'top', caption = "Table 19:Maximum predicted hypolimnetic TDS by site", extensions = "Buttons", options = list(scrollY="400px", scrollX=TRUE, paging=FALSE, dom = 'Bfrtip', buttons = c('csv', 'excel')))

```

```{r, print max LMAS max predicted TDS maps epi and hypo, echo=FALSE, warning=FALSE, message=FALSE, fig.cap= c("Figure 19: Map of actual maximum TDS by site", "Figure 20: Map of predicted maximum TDS by site")}


my_bins <-c(0, 10, 100, 250, 500, 1000, 2000, 3000, 4000, Inf) 
my_pal<-colorBin(palette = "Spectral", bins = my_bins, domain = lakes_TDS_epi_max$`Max TDS (mg/L)`, reverse = TRUE)
...
color = ~my_pal(lakes_TDS_epi_max$`Max TDS (mg/L)`)) %>% 
